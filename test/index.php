...
// $obBasket = \Bitrix\Sale\Basket::getList(array('filter' => array('ORDER_ID' => 36)));
// while($item = $obBasket->Fetch()){
//      echo '<a href="'.$item['DETAIL_PAGE_URL'].'/" target="_blank">'.$item['ID'].'</a><br/>';
//      echo '<img src="'.$item['PREVIEW_PICTURE']['SRC'].'" /> <br/>';
...
<? $rsCart = CSaleBasket::GetList(Array(),Array("ORDER_ID"=>36));?>
<? while ($arCartItem = $rsCart->Fetch()) {?>
        ...
        url: "<?=$arCartItem['DETAIL_PAGE_URL']?>"

        <? 

        $intElementID = $arCartItem["PRODUCT_ID"]; // ID предложения
