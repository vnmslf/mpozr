...
else
{
	...
	<div class="wrap order-list">
		<div class="col">
			...
			{
				?>
				<a class="mr-4 getb" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_VIEW_ORDERS_HISTORY")?></a>
				<?
			}
			if ($_REQUEST["filter_history"] == 'Y')
			{
				?>
				<a class="mr-4 getb" href="<?=$APPLICATION->GetCurPageParam("", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_CUR_ORDERS")?></a>
				<?
				if ($_REQUEST["show_canceled"] == 'Y')
				{
					?>
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $arResult['SORT_TYPE'] == 'STATUS')
			{
				...
				<div class="row mb-3">
					<div class="col">
						<h2 class="h2s"><?= Loc::getMessage('SPOL_TPL_ORDER_IN_STATUSES') ?> &laquo;<?=htmlspecialcharsbx($arResult['INFO']['STATUS'][$orderHeaderStatus]['NAME'])?>&raquo;</h2>
					</div>
				</div>
				<?
			}
			?>
			<div class="onhis">
			<div class="ondate"><?=$order['ORDER']['DATE_INSERT_FORMATED']?></div> 
			<div class="history">

			 <div class="onsum"><?=$order['ORDER']['FORMATED_PRICE']?></div>

			 <div class="onnum"><span>Номер заказа</span> <?=$order['ORDER']['ACCOUNT_NUMBER']?></div>


			<div class="row mx-0 mb-5">
				<div class="col histors">
					<div class="row mb-3 align-items-center zagols">
						<div class="col-auto">
	...
	else
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			...
			<div class="onhis duoble">


			<div class="ondate"><?=$order['ORDER']['DATE_INSERT_FORMATED']?></div> 
			<div class="history">

			 	<div class="onsum"><?=$order['ORDER']['FORMATED_PRICE']?> <div class="stats"><span>Статус</span> Выполнен</div></div>
