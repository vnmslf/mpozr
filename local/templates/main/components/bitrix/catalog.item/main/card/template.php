...
/*		if ($item['LABEL'])
	...
	<div class="priced">
		<span class="del">
			<?
					if ($price['PRINT_RATIO_PRICE'] != 0) {
						...
						//echo 'Расчетная';
						//Вывод цены торговых предложений
						//В случае, если цены одинаковые, то вывести одну цену, если диапазон цен, то вывести минимальную и максимальную
						//Обновление: если цены не указаны или указаны нулевые, то вывести «Расчётная»
						$mxResult = CCatalogSKU::GetInfoByProductIBlock($item['IBLOCK_ID']);
						$offers_prices = array();
						...
						if (is_array($mxResult)) { 
							...
							while ($arOffer = $rsOffers->GetNext()) {
								...
								if ($ar_price['PRICE'] * 1 == true) {
									$offers_prices[] = $ar_price['PRICE'];
								}
							}
						}

						$min_price = min($offers_prices);
						$max_price = max($offers_prices);

						if (count($offers_prices)) {
							if ($min_price == $max_price) {
								$result = CCurrencyLang::CurrencyFormat($min_price, 'RUB', true);
							}
							else {
								$result = 'от '.CCurrencyLang::CurrencyFormat($min_price, 'RUB', true).' до '.CCurrencyLang::CurrencyFormat($max_price, 'RUB', true);
							}
						}
						else {
							$result = 'Расчетная';
		...
		<? if( $item["PROPERTIES"]["strike"]['VALUE'] != ''): ?>
			<span class="sel"><? echo $item["PROPERTIES"]["strike"]['VALUE']; ?> руб.</span>
		<? endif; ?> 
...
<div class="product-item">
	...
	if (!empty($arParams['PRODUCT_BLOCKS_ORDER']))
	{
		foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
		{
			...
			{
				case 'price': ?>
					<div style="display: none;" class="product-item-info-container product-item-price-container" data-entity="price-block">
						<?
				...
				case 'sku':
					...
					{
						?>
						<div style="display:none;" id="<?=$itemIds['PROP_DIV']?>">
							<?
							foreach ($arParams['SKU_PROPS'] as $skuProperty)
							{
								$propertyId = $skuProperty['ID'];
